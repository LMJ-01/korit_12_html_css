...
box-sizing 속성은 속성값으로
  ...
  1. content-box : padding(안쪽 여백)을 제외하고 텍스트 / 이미지 등의 내용이 들어갈 수 있는 영역만 계산함. 예를 들어 div의 width가 400px이라면 내용이 들어갈 수 있는 가로 길이가 440px이라는 의미.
...
2. 값을 두 개로 주는 방법 : 첫 번쨰 속성값은 상단 왼쪽과 하단 오른쪽 / 두 번째 속성값은 상단 오른쪽 / 하단 왼쪽
...
이상의 경우에서 border 속성을 주석처리했을 떄 조금씩 위치가 이동하는 것을 봤을 때, box-sizing의 default 값은 뭐라고 해석할 수 있을까요? -> content-box
...
- overflow의 기본 속성값은 default로 auto로 잡혀있습니다.
  ...
  - auto : 자식 요소가 부모 요소보다 큰 경우 스크롤바를 자동 생성
...
- 속성값 두개 입력할 수도 있는데 첫번째 x축 / 두번째가 y축에 해당
...
div {
  box-shadow: (inset 여부) offset-x(x축 두꼐) offset-y(y축 두께) (blur-radius)(퍼짐효과 크기) (spread-radius)(그림자크기) color
}

실수값이 두 개 일 때 : x축 두께 / y축 두께

실수값이 세 개 일 때 : x축 두께 / y축 두께 / 푸짐효과크기

실수값이 네 개 일 때 : x축 두께 / y축 두께 / 푸짐효과크기 / 그림자 크기 지정
...
너무 복잡하면 chrome에서 box-shadow generator를 검색하세요. - https://html-css-js.com/css/generator/box-shadow/
...
저희는 Java에서 객체의 주소값 할 떄 본 적이 있습니다.
...
근데 얘는 색상의 라인이 무지개처럼 빨주노초파남보 색 거친 다음에 다시 빨간색으로 되돌아가서 360도 회전하는 것처럼 여겨집니다.
...
배경을 넣고자 하는 이미지 파일을 그대로 삽입하거나 해당 이미지에 CSS로 다양한 효과를 적용하는 것이 가능합니다.
...
- 속성값 목록
  ...
  6. round : 배경의 최대 너비와 높이에 맞춰서 그냥 적절하게 계산해서 맞춰줍니다. repeat은 기본 이미지 크기를 그대로 반영하기 떄문에 잘리는 부분이 생기는 반면에  round는 이미지 사이즈를 좀 바꿉니다.
...
1. %값 : 이미지의 중심축이 되는 배경의 어느 시점에 위치해야할지를 결정할 수 있음. 50% / 50% 로 정했을 떄 가로축 50% 지점, 세로축 50% 지점이기 때문에 center로 지정한 것과 동일한 효과를 가지게 됩니다.
...
1. containt : 이미지의 비례와 상관없이 자연스럽게 배경에 맞춰서 이미지를 나타냄.
...
그라데이션 적용 원리는 background 속성에 linear-gradient() 속성값을 지정하고, 괄호내에 argument로 색상을 집어넣습니다. 그런데 색상을 표현하는 방식이 네 가지였죠. 네 가지 다 가능합니다.

### linear-gradient 세 가지 색 지정하기
...
- 원래 있던 위치를 기준으로 top / bottom / left / right / z-index의 위치 속성의 영향을 받도록 할 떄 사용하는 속성값

- 해당 요소가 차지하는 공백의 위치는 그대로 유지됨.

### absolute

- 바로 위 _position 속성의 속성값이 static이 아닌_ 부모 요소를 기준으로 top / bottom / left / right / z-index의 위치 속성의 영향을 받도록 할 떄 사용하는 속성값

- 그래서 만약에 바로 위 부모님 position 속성값이 static이라면 그 윗단계로 넘어갑니다.

- yellow의 postition이 static이고, red의 position을 absolute로 설정한다면, 그 위 단계인 outer div의 위치 값을 따르게 됩니다.
...
- 부모 요소가 아니라 viewprot를 기준으로 위치를 지정합니다. 뷰포트이기 떄문에 스크롤 영향을 받지 않습니다.

- 고정된 위치에서 움직이지 않는다는 특징 때문에 웹사이트에서 어떤 팝업창 뜨워놓고 스크롤해도 해당 위치에 존재하게끔 하거나, nav바에서 주로 사용합니다.
...
- 근데 얘는 부모 요소의 여백에 영향을 받습니다. 자식 요소의 위치를 고정하더라도 부모 요소의 위치를 벗어나는 식으로 사용한다면 그 이상 움직일 수 없습니다.
...
- 내부 자식 요소들은 어느 축을 기준으로 정렬할지 결정함. default 값은 row
...
- display: flex;를 적용했기 때문에 -> flex-direction : row로 정했으면 -> justify-content:flex-start로 잡았다고 가정했을 때, 그러면 자식 요소들이 row로 정렬되는데, 시작점부터니까 왼쪽부터 정렬될 것이다라고 추측할 수 있습니다.
...
- flex의 마이너 축(flex-direction이 row면 column을 의마함)에서 내부 요소를 정렬할 방식을 결정하는 속성.

- default 값으로 stretch가 적용되어있습니다. 즉 flex-direction : row이고, align-items가 stretch라면 마이너 축이 column이니가 column 방향으로 stretch(즉 뻗어나감)된다는 것으로 해석할 수 있습니다.
...
- 그런데 strech 제외 나머지가 48px*48px인 이유는 참조사이트의 설정때문에 그렇습니다.

### flex-wrap

- 내부 요소가 부모의 요 소의 영역을 넘어갈 정도로 많을 경우에 하는 배치 방법
...
![alt text](시맨틱-태그_html-1024x576.webp)