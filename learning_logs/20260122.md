...
`<title>` : 브라우저 상단 탭에 나타나는 사이트 제목에 해당함 - > 북마크 / 즐겨찾기를 했을 떄 저장되는 이름입니다. / 구글과 같은 검색엔진에서 나타나는 검색 결과에 표시되는 제목이기도 함.
...
- `<meta>` :
  ...
  2. viewport : 화면에 보여지는 영역을 의미했었습니다. 모바일 기준으로 봤을 떄는 사람 눈에 보이는 시각적인 뷰포트(viewport)와 브라우저가 웹페이지를 표시하는 영역에 해당하는 레이아웃 뷰포트(layout viewport)가 대체로 동일하기는 합니다. 근데 input 태그 등을 클릭하게 되면 키보드가 올라오게 되는데, 그러면 레이아웃 뷰포트 =/= 시각적 뷰포트가 어긋나는 경우가 생깁니다. 이상과 같은 상황이 생기더라도 웹사이트 내용이 깨지지 않도록 유지시켜주는 부분이라고 할 수 있습니다.

  3. Open Graph 정보
    
    - 메타(전 페이스북)에서 만든 프로토콜로 웹페이지의 주소를 복사해서 페이스북/카카오톡 등의 SNS 등의 계정으로 공유할 떄 보여지는 웹사이트 제목과 썸네일 등의 기본 정보를 태그 형태로 정의하는 방식.

```html
<head>
  <meta property="og:title" content="Korea IT Academy 12월 국비 과정">
  <meta property="og:description" content="심하 HTML / CSS 및 기초 JavaScript 과정">
  <meta property="og:image" content="https://showcases.yalco.kr/html-css-scoop/03-01.png">
</head>
```
와 같이 작성하게 되면 공유했을 떄 썸네일로 이미지, 그리고 조금 두꺼운 폰트로 제목 / 그리고 생략 ...을 포함한 내용이 SNS에 포함되게 됩니다.
...
- div / span 태그만 이용해도 웹페이지를 만드는 것이 가능하지만, 어떤 용도로 사용되는지 구분하는 것이 어렵습니다(그래서 옛날에는 div 태그에 class로 가능이나 용도를 하나하나 다 집어넣기도 했습니다).이 부분을 해결하기 위한 개념이 시맨틱 태그입니다.
...
1. 웹 접근성 개선 : 스크린 리더를 활용하여 웹페이지를 볼 떄 필요한 정보를 빠르게 읽어줍니다.
...
2. `<nav>` : 링크로 이동하는 요소를 포함. 페이지의 메뉴나 색인(index) 등 하이퍼링크가 있는 부분에 사용됩니다. 페이지 내에 id로 지정된 요소가 있다면 그 요소로 바로 이동할 때 사용하기도 합니다(특정 id 클릭해서 넘어가는 부분을 HTML 수업 떄 했습니다).
...
5. `<aside>` : 메인과 어느 정도 간접적으로 연관된 컨텐츠를 포함하는데 사용 되는데, 주로 사이드 바 역할을 합니다.
...
SVG란 ? Scalable Vector Graphics는 일반적인 png나 jpg 와 다른 이미지 파일입니다. 픽셀이미지는 확대할 수록 픽셀 단위가 커지면서 해당도가 떨어지는데, svg는 확대해도 이미지가 깨지지 않는 특징이 있습니다.

1. 이미지 파일 결로를 삽입하는 방법을 쓸 수 있습니다.
<img src = "https://showcases.yalco.kr/html-css-scoop/03-05/yalco-y.svg" alt="알코 개발자님 항상 신세지고 있습니다.">

2. svg 소스를 불러오는 방법
웹 상에 올려져있는 svg 이미지를 불러올 경우에 _이미지를 다른 이름으로 저장_ 을 선택해서 저장합니다. 그리고 팀색기창으로 불러올 떄 소스코드 형식으로 불러오게 될 경우에 
...
<body bgcolor="5f5f5f">
  ...
  <figure>
    ...
    <audio src="http://showcases.yalco.kr/html-css-scoop/03-05/audio.mp3" controls></audio>
  </figure>
  <br>
  <figure>
...
autoplay 속성은 자동재생
loop 반복재생을 위한 속성에 해당합니다. 근데 최근에는 브라우저마다 자동 재생 기능을 막아두기 떄문에 autoplay를 직접 쓰는 일은 잘 없을겁니다.
...
<body bgcolor="5f5f5f">
  ...
  <figure>
    <figcaption>알코님 영상 비디오</figcaption>
    <audio controls>
      <source src="http://showcases.yalco.kr/html-css-scoop/03-05/audio.mp3" type="audio/mpeg">
      <source src="http://showcases.yalco.kr/html-css-scoop/03-05/audio.mp3" type="audio/wav">
...
src 속성을 통해서 집어넣는 방법입니다. 그런데 일부러 여러 예시를 저장했습니다. 윈도우 버전의 edge나 Mac에서는 mov를 또 지원 안합니다. 그래서 브라우저 별로 다르게 지원을 하기 떄문에 동일한 영상이라도 확장자마다 집어넣어야 할 떄도 있습니다. 이상을 이유로 동일한 source를 준비해서 올립니다.
...
그러다보니 태그 이름만 가지고 해당 요소가 사이트의 어떤 부분을 명시하는지 알기가 어려운 경우가 많습니다.

그래서 생겨난 방식으로,

## 클래스 위주로 이름짓기
시멘틱 태그만으로 이름을 짓는 방식에 한계가 있다면 _각 요소 마다 클래스명을 지정하는 방식_ 을 채택할 수 있습니다. 그리고 그 클래스명은 무슨 역할을 하는지 명확하게 적어주는 것이 그 목표입니다.
...
<body bgcolor="5f5f5f"> class="list-item"
  <article class="card">
    <aside class="thumb">
...
- BEM : block element modifier의 축약어로 웹사이트에 있는 요소의 일정 부분을 컴포넌트(component:리엑트에도 나옵니다)로 나눈 후 그것을 블록으로 지정하는 방식입니다.
...
경우에 따라서 이상의 세 가지 방법을 잘섞어서 만드는 것이 중요한데, 조직의 성향을 따라서...
...
CSS에서 변수를 나타낼 때는 원하는 변수명을 지정하고 그 옆에 -- 를 입력해서 변수임을 표시하는 방벙이 있습니다.

페이지 메인 컬러라면 --color-main, 가장 작은 텍스트 크기를 지정하고 싶다면 --font-small과 같은 방식이 됩니다.

ch10의 하위 폴더 cssvariable 폴더를 생성하시오.
02_cssvariable.html / css / common.css 파일을 생성하시오.
...
이상의 두 코드를 고려했을 떄 다른 var()와 달리 var(arg1, arg2)형태로 작성되어있습니다. 이는 사용할 수 있으면 --warn 변수를 사용하고, 없을 경우에는 대안(alt)으로 lightblue를 사용하라는 의미로 작성되었습니다.
...
이상의 코드는 눈누에서 무료 폰트를 가져왔을 떄의 부분입니다. @import가 아니죠.
